...
public class DBAdapter {
	...
	private People[] ConvertToPeople(Cursor cursor) {
		...
		for (int i = 0; i < resultCounts; i++) {
			...
			peoples[i].Name =
					cursor.getString(Math.max(0, cursor.getColumnIndex(KEY_NAME)));
			peoples[i].Age = cursor.getInt(Math.max(0, cursor.getColumnIndex(KEY_AGE)));
			peoples[i].Height =
					cursor.getFloat(Math.max(0, cursor.getColumnIndex(KEY_HEIGHT)));
	...
	private static class DBOpenHelper extends SQLiteOpenHelper {
		...
		private static final String DB_CREATE = "create table " +
				...
				+
				KEY_NAME + " text not null, " + KEY_AGE + " integer," + KEY_HEIGHT +
				" float);";
